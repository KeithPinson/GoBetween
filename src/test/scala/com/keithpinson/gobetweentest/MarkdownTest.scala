...
trait MarkdownTestHelpers {
  ...
  def genLine : Gen[String] = for { cc <- Gen.nonEmptyListOf(Gen.alphaNumChar); e <- Gen.oneOf("\n","\r","\r\n") } yield cc.take(80).mkString + e
...
"""
  ...
  def checkLine = prop( (s:String) => s.last must beOneOf( '\n', '\r' ) ).setGen(genLine)
}
